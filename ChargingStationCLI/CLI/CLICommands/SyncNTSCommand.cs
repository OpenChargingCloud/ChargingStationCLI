...
namespace cloud.charging.open.ChargingStation.CommandLine
{
    ...
    /// <summary>
    /// Ask this station's time servers what the time is, or one of them
    /// everything.
    /// </summary>
    /// <remarks>
    /// Without a server, the same thing the NTS page does with 'Sync now'. Both
    /// end in <see cref="ChargingStation.SyncTimeAsync"/>, which asks the same
    ...
    /// of two stations disagreeing about what they did.
    ///
    /// With one, the same thing as that server's Test button on the page:
    /// <see cref="ChargingStation.TestTimeServerAsync"/>, on the ports the
    /// server is configured with, step by step. Only a server of this station
    ...
    /// </remarks>
    /// <param name="CLI">The command line of the station to ask.</param>
    public class SyncNTSCommand(StationCLI CLI) : ACLICommand<StationCLI>(CLI),
                                                  ICLICommand
    {
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length == 2)
            {
                ...
                var server  = cli.Station.TimeSources.Sources.FirstOrDefault(source => String.Equals(source.Hostname.Trimmed,
                                                                                                     wanted,
                ...
                if (server is null)
                    return [ $"'{Arguments[1]}' is none of this station's time servers, which are {String.Join(", ", TimeServers())}." ];
                ...
                cli.Station.Log.Info(
                    $"Somebody at the command line asked this station to test the time server '{host}'.",
                    "nts", "test", "cli"
                );

                return Tested(await cli.Station.TestTimeServerAsync(host, CancellationToken));
            ...
            // which the station cannot tell apart. Said before anything is
            ...
            cli.Station.Log.Info(
                "Somebody at the command line asked this station to synchronise its time.",
                "nts", "test", "cli"
            );

            var result = await cli.Station.SyncTimeAsync(CancellationToken);
        ...
        /// <summary>
        /// The time servers of this station, switched on or not, as somebody
        ...
        private IEnumerable<String> TimeServers()
            ...
            => cli.Station.TimeSources.Sources.Select(source => source.Hostname.Trimmed);
        ...
        /// and a station in a German locale would otherwise write "+1,2 ms" in
        /// the middle of one.
        /// </summary>
